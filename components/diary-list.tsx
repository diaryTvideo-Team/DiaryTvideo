...
export function DiaryList({ language = "en" }: { language?: Language }) {
  ...
  const filteredEntries = useMemo(() => {
    return entries.filter((entry) => {
      ...
      const matchesDate = !selectedDate || new Date(entry.createdAt).toDateString() === selectedDate.toDateString()
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="mt-4 font-serif text-xl font-semibold text-foreground">{t.noEntriesYet}</h3>
  ...
  return (
    <>
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-6 flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
            <div className="flex-1 max-w-md">
              <DiaryFilter searchQuery={searchQuery} onSearchChange={setSearchQuery} language={language} />
            </div>
          ...
          {filteredEntries.length === 0 ? (
            ...
            <div className={view === "video" ? "grid gap-5 sm:grid-cols-2" : "flex flex-col gap-4"}>
      ...
      <DiaryModal entry={selectedEntry} onClose={() => setSelectedEntry(null)} language={language} />
    </>
  )
}
